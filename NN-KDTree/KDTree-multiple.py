...
def kdTreeNN(filename, outfilename):
    ...
    (totalDist, routeNodes) = kDTreeSearchNN(root, len(points), distSqdMatrix, numNN)
...
def kDTreeSearchNN( tree, numCities, distSqdMatrix, maxNN ):
    start = tree
    target = start
    route = []
    totalDist = 0
