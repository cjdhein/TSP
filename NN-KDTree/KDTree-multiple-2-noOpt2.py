...
#
#     best = (float('inf'), [], [[]])
#     for h in range(0, len(points)):
#         points = [ [int(i[0]), int(i[1]), int(i[2])] for i in [j.split() for j in text ]]
#
#         # Create kd-tree structure with points, 0 start depth, and 2D(x and y)
#         root = kDTree( points, 0, 2)
#         startnode = kDTreeFindById(root, points[h][0])
#
#         (totalDist, routeNodes, distSqdMatrix) = kDTreeSearchNN(root, startnode, len(points), numNN)
#
#         if best[0] > totalDist:
#             newRoute = []
#             for i in routeNodes:
#                 newRoute.append(i.city[0])
#             best = (copy.copy(totalDist), newRoute, distSqdMatrix)
#
#
#     # Save route
#     outFile = open(outfilename, "w")
#     outFile.write(str(best[0]) + "\n")
#     for i in best[1]:
#         outFile.write(str(i) + "\n")
#     return
#
...
#     print(tree.city[0])
#
# def kDTreeFindById( tree, cid):
#     if (tree == None or tree.city[0] == cid):
#         return tree
#
#     # Try right subtree
#     result = kDTreeFindById(tree.left, cid)
#     if result != None:
#         return result
#
#     # Try left subtree
#     result = kDTreeFindById(tree.right, cid)
#     if result != None:
#         return result
#
#     # Failed to find
...
# def kDTreeSearchNN( tree, startnode, numCities, maxNN ):
#     start = startnode
#     target = start
#     route = []
#     totalDist = 0
